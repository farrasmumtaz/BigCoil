    ...
    <>
      ...
      >
        <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-6">
          {/* Logo */}
          <Link to="/">
            <img src={logo} alt="BigKoil" className="h-12 object-contain" />
          </Link>
...
function NavLinkItem({ to, title }) {
  return (
    <NavLink
      to={to}
      className={({ isActive }) =>
        `font-medium transition hover:text-[#9BCFF2] ${
          isActive ? "text-[#9BCFF2]" : "text-stone-700"
        }`
      }
    >
      {title}
...
function LinkItem({ to, text, close, indent = false }) {
  return (
    <Link
      to={to}
      onClick={() => close(false)}
      className={`block border-b border-stone-100 py-4 text-stone-700 hover:text-[#9BCFF2] font-medium transition-colors ${
        indent ? "pl-12 bg-stone-50/50" : "px-6"
      }`}
    >
      {text}
    </Link>
